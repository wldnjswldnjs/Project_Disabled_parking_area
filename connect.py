    ...
    def on_message(self, client, userdata, message):
        try:
            ...
            if message.topic == "mydata/faile": 
                ...
                #모델 적용
                cmd = "python /home/lab28/yolov5/detect.py --weights /home/lab28/yolov5/best.pt --img 416 --conf 0.5 --source {} --save-txt --save-conf".format(val_img_path)
                bashCmd = cmd.split()
                ...
                #점수 산출
                try:
                    with open('/home/lab28/yolov5/runs/detect/exp' + directory_num + '/labels/' + file_name + '.txt','r') as f:
                        lines = f.readlines()
                        lines = list(map(lambda s: s.strip(), lines))
                        score = lines[0].split()

                        if float(score[5]) > 0.7: 
                            print("[점수 산출완료]")
                            msg = "1"
                ...
                publish.single("web/carimg", val_img_path, hostname="3.25.85.102")
                publish.single("picture", msg, hostname="3.25.85.102")
                print("[메세지 송신완료]")
                print("-"*25)
               
            elif message.topic == "mydata/number":
                ...
                cmd = "python /home/lab28/yolov5_license_plate/detect.py --weights /home/lab28/yolov5_license_plate/best.pt --img 416 --conf 0.5 --source {} --save-conf --save-crop".format(val_img_path)
                bashCmd = cmd.split()
                subprocess.Popen(bashCmd).wait(timeout=None)
                print("[모델 적용완료]")
                ...
                subfolders = pd.DataFrame([ f.path for f in os.scandir('/home/lab28/yolov5_license_plate/runs/detect/') if f.is_dir() ], columns = ['dir_path'])
                subfolders_nm = pd.DataFrame([ f.name for f in os.scandir('/home/lab28/yolov5_license_plate/runs/detect/') if f.is_dir() ], columns = ['dir_name'])
